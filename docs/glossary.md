...
| **Impeccable** | A third-party design plugin the installer adds. Its hook reviews UI edits (contrast, fonts, layout) and asks for a fix or a reason. |
...
| **Verify gate** | A hook that stops the coordinator from ending its turn while the unit tests fail. It's armed only after the full check of the builders passes. |
